...
DigitRoll.prototype={
    ...
    roll:function (n) {
        ...
        Array.prototype.forEach.call(this.container.querySelectorAll('.num'), function (item,i) {
            ...
            item.style.cssText += '-webkit-transition-duration:0s;-webkit-transform:translateY(0);transition-duration:0s;transform:translateY(0)';//重置位置
            item.innerHTML = gapStr;
            setTimeout(function () {
                item.style.cssText+='-webkit-transition-duration:1s;-webkit-transform:translateY(-'+self.rollHeight*gapNum+'px);transition-duration:0s;transform:translateY(-'+self.rollHeight*gapNum+'px);';
            },50);
        })
    },
    /**
